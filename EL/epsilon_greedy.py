...
if __name__ == "__main__":
    ...
    def main():
        ...
        for e in epsilons:
            ...
            means = []
            env.max_episode_steps = max(game_steps)
            rewards = agent.play(env) # ここでコイントスが起こっている, 指定されたepsで1エピソード実行
            for s in game_steps:
                means.append(np.mean(rewards[0:s])) #コイントスゲームの回数を10~310で増やしながら比較するため各ゲームの報酬の平均値で比べる
